...
from config import JOURNAL_ABBREVIATIONS
...
def generate_chinese_entries(papers: list[Paper]) -> list[dict]:
...
def build_html(papers: list[Paper], entries: list[dict], run_date: date) -> tuple[str, str, str]:
    ...
    for idx, (paper, entry) in enumerate(zip(papers, entries), start=1):
        ...
        url = paper.url or (f"https://doi.org/{paper.identifier}" if paper.identifier else "")
